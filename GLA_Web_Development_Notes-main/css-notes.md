...
- Use grid for row-and-column layouts.
- Use positioning only when normal flow, flexbox, or grid does not solve the requirement.
...
## Task 1: Typography Card

Create a card containing a heading, paragraph, and link. Apply:
...
Create three boxes and apply different values for:
...
Compare `content-box` with `border-box`.

## Task 3: Display Property

Create navigation links and display them using:

- `inline`
- `inline-block`
- `flex`

Observe the difference in spacing and dimension control.
...
- An absolutely positioned discount badge
- A sticky page header
- A fixed help button

## Task 5: Complete Page

Create a responsive course listing page with:

- External CSS
- Styled headings and paragraphs
- Card box models
- A grid card container
- An absolute card badge
- A sticky header
- A fixed action button
...
## Which type of CSS is recommended for a multi-page website?

External CSS because it provides reusable styles and easier maintenance.

## What is typography in CSS?

Typography is the styling and arrangement of text using properties such as font family, font size, line height, spacing, alignment, and decoration.

## What is the CSS cascade?

The cascade is the process the browser uses to choose the winning declaration when multiple CSS rules apply to the same element.

## What is specificity?

Specificity is the weight calculated from a selector that helps determine which competing declaration is applied.

## Does internal CSS always override external CSS?

No. Importance, specificity, and source order determine the result.
...
Margin is outside the border, while padding is between the content and the border.

## What is the difference between `content-box` and `border-box`?

With `content-box`, declared dimensions apply to content and padding and border are added. With `border-box`, declared dimensions include content, padding, and border.

## What is the difference between `block` and `inline`?

A block element normally starts on a new line and can accept dimensions. An inline element stays within text flow and uses only the space required by its content.

## What is the difference between `display: none` and `visibility: hidden`?

`display: none` removes the element from layout. `visibility: hidden` hides it while generally preserving its layout space.

## What is the difference between relative and absolute positioning?

A relative element remains in normal flow and is offset from its original position. An absolute element is removed from normal flow and positioned using its containing block.

## What is fixed positioning?

Fixed positioning removes an element from normal flow and positions it relative to the viewport.

## What is sticky positioning?

Sticky positioning behaves like relative positioning until a scrolling threshold is reached, after which it sticks within its scrolling context.

## What is `z-index`?

`z-index` helps define the stacking order of overlapping elements within applicable stacking contexts.
...
```text
Cascade:
Origin and importance -> Specificity -> Source order
Inheritance may supply values from a parent
```
...
These CSS concepts form the foundation of webpage styling and layout:

- **Types of CSS** define where styles are written.
- **Typography** controls the appearance and readability of text.
- **The cascade** resolves competing style declarations.
- **The box model** determines element dimensions and spacing.
- **Display** controls an element's layout behavior.
- **Positioning** controls how elements are placed and layered.

Understanding these topics makes it easier to create clean, responsive, and maintainable user interfaces.
